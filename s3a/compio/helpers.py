...
class IOTemplateManager:
  ...
  @classmethod
  def getTableData(cls, ioType: str):
    cfgFname = ioType.lower() + '.tblcfg'
    cfg = cls.templates.get(cfgFname)
    if cfg is None:
      return None
    td = TableData(cfgFname=cfgFname, cfgDict=cfg, requiredFields=[])
    return td

  @classmethod
  def registerTableData(cls, ioType: str, cfg: t.Union[FilePath, dict], force=False):
    """
